    ...
    <div class="container mx-auto px-4 py-3 flex items-center gap-4">
      ...
      <div v-if="currentEpisode" class="hidden md:flex items-center gap-3">
        <div class="w-12 h-12 rounded-lg overflow-hidden">
          <img 
            :src="currentEpisode.thumbnail || 'https://picsum.photos/200/200?random=1'" 
            :alt="currentEpisode.title" 
            class="w-full h-full object-cover"
          >
        </div>
        <div>
          <h4 class="text-sm font-medium truncate max-w-48">{{ currentEpisode.title }}</h4>
          <p class="text-xs text-slate-500">{{ currentEpisode.author || 'Web Worker' }}</p>
        </div>
      </div>
