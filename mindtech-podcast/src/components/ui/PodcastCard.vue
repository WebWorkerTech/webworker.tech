<template>
  <article class="group bg-white rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
    <div class="relative">
      <img 
        :src="episode.thumbnail || episode.imageUrl || 'https://picsum.photos/600/400?random=6'" 
        :alt="episode.title" 
      ...
      <div class="absolute top-4 right-4 bg-slate-900/80 text-white text-xs px-2 py-1 rounded">
    ...
    <div class="p-6">
      <div class="flex items-center gap-2 mb-3">
        ...
        <span class="text-xs text-slate-400">{{ formatDate(episode.pubDate) }}</span>
      </div>
      
      <h3 class="text-xl font-bold mb-2 group-hover:text-mint-400 transition-colors line-clamp-2">
        {{ episode.title }}
      </h3>
      <p class="text-slate-600 mb-4 line-clamp-2" v-html="getPlainDescription(episode.description)"></p>
      
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
          <div class="w-8 h-8 rounded-full bg-mint-100 flex items-center justify-center">
            <Mic class="w-4 h-4 text-mint-400" />
          </div>
          <span class="text-sm font-medium">{{ episode.author || 'Web Worker' }}</span>
        </div>
        <button 
