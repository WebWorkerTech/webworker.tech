<template>
  <section class="mb-16 bg-gradient-to-br from-mint-100 to-cream-200 rounded-3xl p-8 md:p-12 relative overflow-hidden">
    ...
    <div class="relative max-w-2xl mx-auto text-center">
      ...
      <p class="text-slate-600 mb-8">每周更新，不错过任何一期精彩内容。你可以通过以下平台订阅我们的播客：</p>
      ...
      <div class="bg-white p-6 rounded-xl shadow-lg">
        <h3 class="text-lg font-bold mb-4">获取播客更新邮件</h3>
        <p class="text-slate-500 text-sm mb-4">我们会每周发送一期精选内容摘要到你的邮箱</p>
        ...
        <form @submit.prevent="subscribe" class="flex flex-col sm:flex-row gap-3">
          <input 
            ...
            required
            class="flex-1 px-4 py-3 rounded-full border border-cream-200 focus:outline-none focus:ring-2 focus:ring-mint-400/50"
          >
          <button 
            type="submit" 
            class="bg-mint-400 text-white px-6 py-3 rounded-full hover:bg-mint-500 transition-colors"
            :disabled="isSubscribing"
          >
            {{ isSubscribing ? '订阅中...' : '订阅邮件' }}
          </button>
        </form>
        
        <p class="text-xs text-slate-400 mt-4">我们尊重你的隐私，不会向第三方分享你的信息</p>
      </div>
    </div>
  </section>
</template>
...
import { ref } from 'vue';
import { Smartphone, Music, Radio, Rss } from 'lucide-vue-next';
