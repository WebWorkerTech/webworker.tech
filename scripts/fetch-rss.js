...
async function parseRSSXML(xmlContent) {
  ...
  for (const itemXml of itemMatches) {
    ...
    const description = extractCDATA(
      extractXMLData(itemXml, 'description')[0] || '',
    )
    const content = extractCDATA(
      extractXMLData(itemXml, 'content:encoded')[0] || description,
    )
    ...
    const author =
      extractXMLData(itemXml, 'author')[0] ||
      extractXMLData(itemXml, 'itunes:author')[0] ||
      ''
  ...
  try {
    ...
    try {
      ...
      const response = await fetch(
        `https://api.rss2json.com/v1/api.json?rss_url=${encodeURIComponent(RSS_URL)}&api_key=&count=50`,
      )
